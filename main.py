...
class Bouton:
    ...
    def Afficher_famille(self):
        global familles
        self.afficher()
        #fonction qui affiche uniquement les element de la famille hover
        #sinon affiche toutes les familles
        for e in elements:
            if e.famille == self.nom:
                e.afficher2(e.px,e.py)
                if (selected.famille == self.nom):
